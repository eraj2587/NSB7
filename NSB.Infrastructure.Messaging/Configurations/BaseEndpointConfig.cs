...
using NSB.Infrastructure.Logging;
...
namespace NSB.Infrastructure.Messaging.Configurations
{
    public abstract class BaseEndpointConfig : IEndpointConfig
    {
        ...
        protected virtual string GetConnectionStringConfigNameFromEndpointName(string endpointName)
        {
            ...
            if (parts.Length < 3 ||
                !parts[0].EndsWith("NSB", StringComparison.OrdinalIgnoreCase) ||
                !parts[1].Equals("Endpoints", StringComparison.OrdinalIgnoreCase))
            {
                throw new ArgumentException(string.Format("Endpoint '{0}' doesn't follow convention NSB.Endpoints.*. Can't guess the connection string suffix for this endpoint.", endpointName), "endpointName");
            }
