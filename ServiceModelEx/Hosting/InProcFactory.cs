...
namespace ServiceModelEx
{
    public static class InProcFactory
    {
        ...
        private static IServiceBehavior[] defaultServiceBehaviors = new IServiceBehavior[0];
        private static Type[] defaultGenericResolverTypes = new Type[0];
        ...
        static InProcFactory()
        {
            ...
            Binding = binding;
            //binding.MaxReceivedMessageSize *= 12;
                                                                                                                    ...
                                                                                                                    where S : class, I
        {
            ...
            else
            {
                ...
                if (Scope == null) throw new InvalidOperationException(string.Format("Scope is not set yet when trying to instantiate {0}", typeof(I).FullName));
        ...
        public static void SetDefaultServiceBehaviour(params IServiceBehavior[] serviceBehaviors)
        {
            if (m_Hosts.Any()) throw new InvalidOperationException("InProcFactory already has open hosts. Cannot set default service behaviors at this time");
            defaultServiceBehaviors = serviceBehaviors;
        }

        public static void SetGenericResolver(Type[] types)
        {
            if (m_Hosts.Any()) throw new InvalidOperationException("InProcFactory already has open hosts. Cannot set default service behaviors at this time");
            defaultGenericResolverTypes = types;
        }
    }
}