...
namespace ServiceModelEx
{
    public class GenericResolver : DataContractResolver
    {
        ...
        {
            return GetDataContractNamespace(type) ?? type.Namespace ?? DefaultNamespace;
        }

        private static string GetDataContractNamespace(Type type)
        {
            var dataContractAttributes = type.GetCustomAttributes(typeof(DataContractAttribute), false);
            if (dataContractAttributes.Any())
            {
                var dataContractAttribute = (DataContractAttribute)dataContractAttributes.First();
